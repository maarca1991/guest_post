...
if(isset($_REQUEST['submit']))
{
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$db->rplocation(ADMINURL."manage-vendor-team-url-link/".$url_id);
		exit;
	}
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add")
	{
		...
		$row5 = array(
			"to_id" 		=> $client_id,
			"from_id"		=> $_SESSION[SESS_PRE.'_VENDOR_TEAM_SESS_ID'],
		...
		if(ISMAIL)
		{
			...
			$nt = new Notification();
			include("../mailbody/vendor_team_add_link_to_client.php");
		...
		if(ISMAIL)
		{
			...
			$nt = new Notification();
			include("../mailbody/vendor_team_add_link_to_seo.php");
			$subject	= SITETITLE." Added new link";
		...
		if($seo_team_id != null){
			...
			if(ISMAIL)
			{
				...
				$nt = new Notification();
				include("../mailbody/vendor_team_add_link_to_seoteam.php");
				$subject	= SITETITLE." Added new link";
				$nt->rpsendEmail($seo_team_id,$subject,$body);
			}
		}

		// Send notification to vendor for added new link by team members
		$vendor_id = $db->rpgetValue("admin","vendor_id"," isDelete = 0 AND id=".$_SESSION[SESS_PRE.'_VENDOR_TEAM_SESS_ID']);
		$row5 = array(
			"to_id" 		=> $vendor_id,
			"from_id"		=> $_SESSION[SESS_PRE.'_VENDOR_TEAM_SESS_ID'],
			"subject_name"	=> "Added New Link By Team Member",
			"description"	=> "You have received new link which is added by team member.",
			"link"			=> "add-vendor-url-link/edit/".$url_link_id
		);
		$db->minsert("notification",$row5);
		if(ISMAIL)
		{
			...
			$nt = new Notification();
			include("../mailbody/vendor_team_to_vendor.php");
			$subject	= SITETITLE." Added new link";
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete")
{
	...
	$db->rplocation(ADMINURL."manage-vendor-team-url-link/");
	exit;
}
?>
<!DOCTYPE html>
					...
					<form role="form" name="frm" id="frm" action="." method="post" enctype="multipart/form-data">
						...
						<div class="box-body">
							...
							<div class="box-footer">
								...
								<button type="button" class="btn btn-inverse waves-effect w-md waves-light" onClick="window.location.href='<?= ADMINURL ?>manage-vendor-team-url-link/<?php echo $url_id; ?>'">Back</button>
							</div>
						</div>
					</form>
				</div>
