...
if(isset($_SESSION[SESS_PRE.'_ADMIN_SESS_ID']) || isset($_SESSION[SESS_PRE.'_SEO_SESS_ID']))
{
...
if(isset($_REQUEST['submit']))
{
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$db->rplocation(ADMINURL."manage-seo-team/");
		exit;
	}
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add")
	{
		...
		else
		{
			...
			$seo_team_id = $db->minsert($ctable,$rows);
			
			if($seo_team_id > 0 && ISMAIL)
			{
				$subject = SITETITLE."Activate Your Account";
				$nt = new Notification();
				include("../mailbody/admin_created_seo_team.php");
				$toemail = $email;
						...
						<div class="box-body">
							...
							<div class="row">
								<?php if(isset($_SESSION[SESS_PRE.'_ADMIN_SESS_ID'])){ ?>
									<div class="col-md-6">
										<label for="seo_id">SEO<code>*</code></label>
										<select class="form-control" id="seo_id" name="seo_id">
											...
											<?php
												...
												while($seo_d = @mysqli_fetch_assoc($seo_r))
												{
													...
													if( $seo_d['id'] == $seo_id )
														echo ' selected';
													echo '>' . $seo_d['email'] . '</option>';
												}
											?>
										</select>
									</div>
								<?php }else{ ?>
									<input type="hidden" class="form-control" id="seo_id" name="seo_id" value="<?php echo $_SESSION[SESS_PRE.'_SEO_SESS_ID']; ?>">
								<?php } ?>
								<div class="col-md-6">
							...
							<div class="box-footer">
								...
								<button type="button" class="btn btn-inverse waves-effect w-md waves-light" onClick="window.location.href='<?= ADMINURL ?>manage-seo-team/'">Back</button>
							</div>
						</div>
					</form>
				</div>
	...
	$(function(){
		$("#frm").validate({
			...
			messages: {
				...
				seo_id:{required:"Please select seo."}
			}
		});
	});
