...
<form action="" name="frm" id="frm" method="post">
	...
	<input type="hidden" name="hdndb" value="<?php echo $ctable; ?>">
		...
		?>
		<button type="button" class="btn btn-inverse sidebar m-t-10" style="margin-bottom: 5px;" onclick="window.location.href='<?php echo ADMINURL; ?>manage-client-url/'" title="back"><i class="fa fa-reply"></i></button>
					...
					<tr>
						...
						<td><?php
							if($ctable_d['clientEditDate'] != null){ 
								...
								echo "<b style='color:#00a65a;'>".$db->rpgetValue("admin","name","id=".$client_id);
							}elseif($ctable_d['seoEditDate'] != null){
								$seo_id = $db->rpgetValue("url","seo_id","id=".$ctable_d['url_id']);
								echo "<b style='color:#367fa9;'>".$db->rpgetValue("admin","name","id=".$seo_id);
							}elseif($ctable_d['seoTeamEditDate'] != null){
								$seo_team_id = $db->rpgetValue("url","seo_team_id","id=".$ctable_d['url_id']);
								echo "<b style='color:#367fa9;'>".$db->rpgetValue("admin","name","id=".$seo_team_id); 
							}else{
								echo "-";
							}
						?></td>
						...
						<td>
							...
							<a class="btn btn-xs btn-icon waves-effect waves-light btn-primary m-b-5" href="<?php echo ADMINURL?>add-client-url-link/edit/<?php echo $ctable_d['id']; ?>/" title="Edit Link"><i class="fa fa-pencil"></i></a>
							<?php }else{ ?>
							<a class="btn btn-xs btn-icon waves-effect waves-light btn-success m-b-5" href="<?php echo ADMINURL?>add-client-url-link/view/<?php echo $ctable_d['id']; ?>/" title="View Link"><i class="fa fa-eye"></i></a>
							<?php 
							$where = " isDelete = 0 AND link_id=".$ctable_d['id'];
							$check_r = $db->rpgetData("link_content","*",$where);
							$check_d = @mysqli_fetch_array($check_r);
							if($check_d > 0){ ?>
								<a class="btn btn-xs btn-icon waves-effect waves-light btn-primary m-b-5" href="<?php echo ADMINURL?>manage-client-content/<?php echo $ctable_d['id']; ?>/" title="Manage Content"><i class="fa fa-plus"></i></a>
							<?php } ?>
							<?php } ?>
						</td>
					</tr>
