...
if(isset($_REQUEST['submit']))
{
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		$array = array(
			"status" 	=> $status,
			"seoTeamEditDate" => date('Y-m-d H:i:s')
		);
		...
		$row5 = array(
			"to_id" 		=> $vendor_id,
			"from_id"		=> $_SESSION[SESS_PRE.'_SEO_TEAM_SESS_ID'],
			"subject_name"	=> "Approval From seo team member side",
	...
	$db->rplocation(ADMINURL."manage-seo-team-url-link/".$url_id);
}
					...
					<form role="form" name="frm" id="frm" action="." method="post" enctype="multipart/form-data">
						...
						<div class="box-body">
							...
							<div class="box-footer">
								...
								<?php } ?>
								<button type="button" class="btn btn-inverse waves-effect w-md waves-light" onClick="window.location.href='<?= ADMINURL ?>manage-seo-team-url-link/<?php echo $url_id; ?>'">Back</button>
							</div>
						</div>
					</form>
				</div>
