...
if(isset($_SESSION[SESS_PRE.'_ADMIN_SESS_ID']) || isset($_SESSION[SESS_PRE.'_VENDOR_SESS_ID']))
{
...
if(isset($_REQUEST['submit']))
{
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$db->rplocation(ADMINURL."manage-vendor-team/");
		exit;
	}
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add")
	{
		...
		else
		{
			...
			$vendor_team_id = $db->minsert($ctable,$rows);
			
			if($vendor_team_id > 0 && ISMAIL)
			{
				$subject = SITETITLE."Activate Your Account";
				$nt = new Notification();
				include("../mailbody/admin_created_vendor_team.php");
				$toemail = $email;
					...
					<form role="form" name="frm" id="frm" action="." method="post" enctype="multipart/form-data">
						...
						<div class="box-body">
							...
							<div class="row">
								...
								<div class="col-md-6">
									<label for="vendor_id">Vendor<code>*</code></label>
										...
										<?php
											...
											while($vendor_d = @mysqli_fetch_assoc($vendor_r))
											{
												...
												if( $vendor_d['id'] == $vendor_id )
													echo ' selected';
												echo '>' . $vendor_d['email'] . '</option>';
											}
										?>
									</select>
								</div>
								<?php }else{ ?>
									<input type="hidden" class="form-control" id="vendor_id" name="vendor_id" value="<?php echo $_SESSION[SESS_PRE.'_VENDOR_SESS_ID']; ?>">
								<?php } ?>
								<div class="col-md-6">
							...
							<div class="box-footer">
								...
								<button type="button" class="btn btn-inverse waves-effect w-md waves-light" onClick="window.location.href='<?= ADMINURL ?>manage-vendor-team/'">Back</button>
							</div>
						</div>
					</form>
				</div>
