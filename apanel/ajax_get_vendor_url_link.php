...
<form action="" name="frm" id="frm" method="post">
	...
	<input type="hidden" name="hdndb" value="<?php echo $ctable; ?>">
		...
		<?php
			// $db->getDeleteButton();
			// echo $db->getAddButton1("vendor-url-link",$ctable1);
		?>
		<a class="btn btn-primary sidebar m-t-10" style="margin-bottom: 5px;" href="<?php echo ADMINURL?>add-vendor-url-link/add/<?php echo $url_id; ?>"><i class="fa fa-plus"></i></a>

		<button type="button" class="btn btn-inverse sidebar m-t-10" style="margin-bottom: 5px;" onclick="window.location.href='<?php echo ADMINURL; ?>manage-vendor-url/'" title="back"><i class="fa fa-reply"></i></button>
			...
			<tbody>
				...
				if(@mysqli_num_rows($ctable_r)>0){
					...
					if($ctable_d['liveStatus'] == 0){
						...
						$status = '<div class="btn btn-xs btn-icon waves-effect waves-light btn-primary m-b-5" title="Live"><i class="fa fa-heart"></i></div>';
					}
					?>
					<tr>
						<!-- <td><input type="checkbox" name="chkid[]" value="<?php echo $ctable_d['id']; ?>"></td> -->
						<td><?php echo $count+$page_position; ?></td>
						<td><?php echo $db->rpgetValue("url","url","id=".$ctable_d['url_id']); ?></td>
						...
						<td><?php 
							if($ctable_d['clientEditDate'] != null){ 
								...
								$seo_id = $db->rpgetValue("url","seo_id","id=".$ctable_d['url_id']);
								echo "<b style='color:#367fa9;'>".$db->rpgetValue("admin","name","id=".$seo_id)."</b>";
						...
						<td>
							...
							<a class="btn btn-xs btn-icon waves-effect waves-light btn-primary m-b-5" href="<?php echo ADMINURL?>add-vendor-url-link/edit/<?php echo $ctable_d['id']; ?>/" title="Edit Link"><i class="fa fa-pencil"></i></a>
							...
							<?php }else{ ?>
								<a class="btn btn-xs btn-icon waves-effect waves-light btn-success m-b-5" href="<?php echo ADMINURL?>view-url-link/view/<?php echo $ctable_d['id']; ?>/" title="View Link"><i class="fa fa-eye"></i></a>
								<?php if($ctable_d['clientEditDate'] != null || $ctable_d['seoEditDate'] != null || $ctable_d['seoTeamEditDate'] != null){
									if($ctable_d['status'] == 1){
								 ?>
									<a class="btn btn-xs btn-icon waves-effect waves-light btn-primary m-b-5" href="<?php echo ADMINURL?>manage-vendor-content/<?php echo $ctable_d['id']; ?>/" title="Add Content"><i class="fa fa-plus"></i></a>
									<?php  
									$check_content = $db->rpgetValue("link_content","id","isDelete=0 AND link_id=".$ctable_d['id']." AND status=1");
									if($check_content != null){ ?>
										<?php if($ctable_d['liveStatus'] == 0 ){ ?>
										<a class="btn btn-xs btn-icon waves-effect waves-light btn-warning m-b-5" href="<?php echo ADMINURL?>vendor-live-link/live/<?php echo $ctable_d['id']; ?>/" title="Live Link"><i class="fa fa-upload"></i></a>
								<?php } } } } ?>
							<?php } ?>
						</td>
					</tr>
		...
		<?php 
			...
			// $db->getDeleteButton();
			// $db->getAddButton("vendor-url-link",$ctable1);
		?>
		<a class="btn btn-primary sidebar m-t-10" style="margin-bottom: 5px;" href="<?php echo ADMINURL?>add-vendor-url-link/add/<?php echo $url_id; ?>"><i class="fa fa-plus"></i></a>
