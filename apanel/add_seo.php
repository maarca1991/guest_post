...
if(isset($_REQUEST['submit']))
{
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$db->rplocation(ADMINURL."manage-seo/");
		exit;
	}
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add")
	{
		...
		else
		{
			...
			$seo_id = $db->minsert($ctable,$rows);
			
			if($seo_id > 0 && ISMAIL)
			{
				$subject = SITETITLE."Activate Your Account";
				$nt = new Notification();
				include("../mailbody/admin_created_seo.php");
					...
					<form role="form" name="frm" id="frm" action="." method="post" enctype="multipart/form-data">
						...
						<div class="box-body">
							...
							<div class="box-footer">
								...
								<button type="button" class="btn btn-inverse waves-effect w-md waves-light" onClick="window.location.href='<?= ADMINURL ?>manage-seo/'">Back</button>
							</div>
						</div>
					</form>
				</div>
