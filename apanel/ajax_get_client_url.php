...
$ctable_where .= " isDelete=0 AND client_id=".$_SESSION[SESS_PRE.'_CLIENT_SESS_ID'];
...
<form action="" name="frm" id="frm" method="post">
	...
	<input type="hidden" name="hdndb" value="<?php echo $ctable; ?>">
		...
		<?php
			$db->getDeleteButton();
			echo $db->getAddButton1("client-url",$ctable1);
		?>
					...
					<tr>
						...
						<td>
							<?php if($ctable_d['vendor_id'] == null){ ?>
                    			<a class="btn btn-xs btn-icon waves-effect waves-light btn-primary m-b-5" href="<?php echo ADMINURL?>add-client-url/edit/<?php echo $ctable_d['id']; ?>/" title="Edit"><i class="fa fa-pencil"></i></a>
							...
							if($check_d > 0){ ?>
								<a class="btn btn-xs btn-icon waves-effect waves-light btn-success m-b-5" href="<?php echo ADMINURL?>manage-client-url-link/<?php echo $ctable_d['id']; ?>/" title="Url Link"><i class="fa fa-link"></i></a>
							<?php } ?>
						</td>
					</tr>
					<?php
		...
		<?php 
			...
			$db->getDeleteButton();
			$db->getAddButton("client-url",$ctable1);
		?>
</br>
</form>
