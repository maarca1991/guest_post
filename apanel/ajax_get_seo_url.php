...
$ctable_where .= " isDelete=0 AND seo_id=".$_SESSION[SESS_PRE.'_SEO_SESS_ID'];
...
<form action="" name="frm" id="frm" method="post">
	...
	<input type="hidden" name="hdndb" value="<?php echo $ctable; ?>">
		...
		<?php
			// $db->getDeleteButton();
			// echo $db->getAddButton1("seo-url",$ctable1);
		?>
			...
			<tbody>
				...
				if(@mysqli_num_rows($ctable_r)>0){
					...
					<tr>
						...
						<td><?php if($ctable_d['seo_team_id'] == null){ echo "-"; }else{ echo $db->rpgetValue("admin","name"," id=".$ctable_d['seo_team_id']); } ?></td>
						<td><?php  if($ctable_d['seo_team_assign_date'] != null){ echo $db->rpDate($ctable_d['seo_team_assign_date'],"m/d/Y H:i A"); } else { echo "-"; } ?></td>
						<td>
							<?php if($ctable_d['seo_id'] == null){ ?>
							...
							if($check_d > 0){ ?>
								<a class="btn btn-xs btn-icon waves-effect waves-light btn-success m-b-5" href="<?php echo ADMINURL?>manage-seo-url-link/<?php echo $ctable_d['id']; ?>/" title="Url Link"><i class="fa fa-link"></i></a>
							<?php } ?>
							<a class="btn btn-xs btn-icon waves-effect waves-light btn-default m-b-5" href="<?php echo ADMINURL?>assign-url-seo-team/assign/<?php echo $ctable_d['id']; ?>/" title="Assign Team Member"><i class="fa fa-user"></i></a>
						</td>
					</tr>
					<?php
					}
		...
		<?php 
			...
			// $db->getDeleteButton();
			// $db->getAddButton("seo-url",$ctable1);
		?>
</br>
</form>
